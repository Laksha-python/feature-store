...
def compute_user_features(events,reference_time=None):
    ...
    if reference_time is None:
        valid_times=[
            parse_time(e.get("event_time"))
            for e in events
            if parse_time(e.get("event_time")) is not None
        ]
        ...
        reference_time=max(valid_times)
    ...
    for event in events:
        ...
        if (
            event_type=="purchase"
            and event_time>=window_7d
        ):
            purchase_7d[user]+=1
        if (
            event_type=="purchase"
            and event_time>=window_30d
        ):
            try:
                ...
                pass
        if (
            user not in last_event_time
            or event_time>last_event_time[user]
        ):
    ...
    return {
        "rolling_7d_purchase_count": {
            u: purchase_7d.get(u,0)
            for u in all_users
        },

        "rolling_30d_spend": {
            u: spend_30d.get(u,0.0)
            for u in all_users
        },
...
def compute_product_features(events, reference_time=None):
    ...
    for product in all_products:
        ...
        total_interactions=total_views+total_purchases
        conversion_rate[product]=(
            round(total_purchases/total_interactions,4)
            if total_interactions>0
            else 0
        )

        refund_rate[product]=(
            round(total_refunds/total_purchases,4)
...
def compute_net_revenue_features(events):
